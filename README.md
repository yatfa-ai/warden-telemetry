...
#   },
#   "seenKeys": {
#     "configured": { "maxKeys": 10000, "ttlMs": 600000 },  # FIFO cap + per-key TTL backing the dedup set
#     "size": 7                              # live keys in the set — the dedup CAPACITY (upper bound; expired entries purge lazily)
#   },
#   "unreadable": 0,                          # lines on disk that failed to parse this read (read-path signal loss the write-path tallies miss); state snapshot, self-heals to 0 on the next compaction
#   "timeline": {
#     "buckets": [
...
misconfiguration detector need not survive a restart. (`seenKeys` and `unreadable` are the exceptions, not
tallies: `seenKeys` is the idempotent-ingest dedup SET, persisted beside `telemetry.ndjson` (WARDEN-803)
so a retried batch whose 2xx was lost before a restart still dedups — see `deduped` below for the dedup
correctness story; `unreadable` reads the on-disk file too, so it also survives a restart — see its own
subsection below.) That design has a hole without `startedAt`: a
...
`seenKeys` is the capacity-health complement to `deduped`: the live FILL LEVEL of the in-memory set that backs the dedup decision (WARDEN-790). `deduped` tells you the dedup **fired** (a hit count); `seenKeys` tells you the set that backs it **can still catch the next retry** (a fill level). It reports `configured` (the FIFO `maxKeys` cap + the per-key `ttlMs` the set was built with) and the current `size`. The set is bounded by BOTH a count cap (when a new key would exceed `maxKeys`, the OLDEST insertion-ordered key is evicted — FIFO) and a TTL (each key expires `ttlMs` after it was recorded; expiry is observed lazily on access). Under either pressure the dedup can silently DEGRADE: a fleet of clients — or a client-side idempotency-key bug emitting near-unique keys per batch — can pin the set at the cap, so a retried batch whose key was evicted before its retry arrives is treated as FRESH and persisted again, inflating `/summary` + `/events` while `deduped` still climbs on the keys that did hit; likewise a retry landing after the `ttlMs` window is silently re-persisted. `size` against `maxKeys` makes that legible instead of silent. Note `size` is an UPPER BOUND on live keys — expired entries are purged only lazily on access, so a stale-looking high read means "the set WAS that full," never a false alarm of its own. Pair it with `deduped` exactly as `retention.retainedCount` pairs with `retention`'s configured cap: hit-count + capacity together. Like `deduped`, it is receiver-local, does not survive a restart, and is purely additive — it reads a value the set already computes, relaxes no check, and carries only counts + bounds (no raw keys, no payloads). An absent `seenKeys` dep yields the zeroed shape (`configured: { maxKeys: 0, ttlMs: 0 }, size: 0`).

`unreadable` is the read-path signal-loss count — the one gap the write-path tallies above do NOT cover (WARDEN-825). An event can clear EVERY write-path gate — it validated (so it never entered `rejections`), it persisted (so it never entered `persistErrors`), and it survived retention (so it was never `retention`-pruned) — yet still VANISH from the maintainer's signal with zero diagnostic, because its line became **unreadable on disk**. The cause is concrete, not speculative: `createReceiver` has no `SIGTERM`/`SIGINT` handler, so a container or systemd stop while `fileSink`'s non-atomic `appendFile` is mid-flight (or a debounced compaction is running) leaves the exact partial line that `parseNdjson` already defends against — it SKIPS the bad line and keeps the good records on either side of it. The defense (skip) has always existed; `unreadable` is the observability for it: a count of the lines that failed to parse during THIS read. It exists so a maintainer can tell **signal is silently dropping** (a non-zero count — events are being written but some are un-readable, so `/summary` undercounts the real on-disk line count) apart from **a clean store** (the count is `0`). `total + unreadable` reconciles with the on-disk non-blank line count, closing the silent undercount where a skipped line otherwise drops out of `total` with no entry anywhere. The count is a STATE snapshot recomputed per read (NOT a cumulative tally like `rejections`/`persistErrors`/`retention`/`deduped` — every `/summary` request re-reads the file and re-skips the same line, so a cumulative counter would inflate on every read); and it SELF-HEALS to `0` on the next retention compaction, because the compaction re-serializes only the PARSED events and drops the unparseable line, so the corruption is transient-by-design. Unlike the in-memory tallies (which `startedAt` exists to disambiguate), `unreadable` reads the persistent file — it SURVIVES a restart and reads the same count across reboots until a compaction rewrites the file. The trust posture is identical to the sibling tallies and stricter on one axis: it carries a bare integer COUNT only and NEVER echoes the corrupt line's bytes (a partial line could carry a payload fragment or a residual identifier), so the entire surface is the number. It is receiver-local, computed in memory off the on-disk file, purely additive (it counts skips that already happen, reads nothing new, relaxes no check, routes nothing anywhere), and always present — a clean store reads `0`.
