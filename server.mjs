...
//
// The receiver owns its route: POST /ingest. The client POSTs the batch verbatim
// to its configured endpointUrl (e.g. http://host:7421/ingest) and never rewrites
// the host, so this route path is the receiver's to define.

import { createServer } from 'node:http';
import { fileURLToPath } from 'node:url';
import { SCHEMA_VERSION, validateEvent } from './schema.ts';
import { createNdjsonStore, fileSink } from './store.mjs';
...
export function createReceiver({
  ...
  store = createNdjsonStore({ sink: fileSink(storePath) }),
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  server.on('listening', () => {
    ...
    console.log(
      `warden-telemetry receiver listening on :${port} (POST ${INGEST_PATH}; store: ${storePath})`
    );
  });
}
