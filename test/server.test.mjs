...
test('GET /summary returns the aggregate over a pre-populated store → 200 + JSON', async () => {
  ...
  assert.deepEqual(body.byType, { error: 1, crash: 1, 'performance-stall': 0, 'operational-metrics': 0, 'server-stall': 0, 'workspace-names': 0, 'workspace-shape': 0 });
...
test('GET /summary on an empty store → 200, total: 0, zeroed counters (not an error)', async () => {
  ...
  assert.deepEqual(body.byType, { error: 0, crash: 0, 'performance-stall': 0, 'operational-metrics': 0, 'server-stall': 0, 'workspace-names': 0, 'workspace-shape': 0 });
...
test('GET /summary?type=crash scopes the aggregates to a single base type', async () => {
  ...
  assert.equal(body.matched, 2);
  assert.deepEqual(body.byType, { error: 0, crash: 2, 'performance-stall': 0, 'operational-metrics': 0, 'server-stall': 0, 'workspace-names': 0, 'workspace-shape': 0 });
...
test('GET /summary?appVersion=0.1.18&platform=darwin&type=crash intersects all filters (regression attribution)', async () => {
  ...
  assert.deepEqual(body.byType, { error: 0, crash: 1, 'performance-stall': 0, 'operational-metrics': 0, 'server-stall': 0, 'workspace-names': 0, 'workspace-shape': 0 });
